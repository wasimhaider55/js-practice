let score = 33; 
let name = "wasim haider";

// console.log(typeof score);
// console.log(typeof(score));
// console.log(typeof(name));

let count = "44";

let valueInNumber = Number(count);
// console.log(typeof(valueInNumber));


let counter = "444abc"

let valueOfNumber = Number(counter);
// console.log(typeof(valueOfNumber));
// console.log(valueOfNumber); // NaN not a number 

// "33" => 33
// "33abc" => NaN
//  true => 1; false => 0


let isLoggedIn = "wasim";

let BooleanIsLogin = Boolean(isLoggedIn);
// console.log(BooleanIsLogin);


// 1 => true; 0 => false
// "" => false
// "hitesh" => true

let someNumber = 33;

let stringNumber = String(someNumber)
// console.log(stringNumber);
// console.log(typeof stringNumber);

// *********************** Operations ***********************

let value = 3
let negValue = -value
// console.log(negValue);

// console.log(2+2);
// console.log(2-2);
// console.log(2*2);
// console.log(2**2);
// console.log(2**3);
// console.log(2/3);
// console.log(2%3);

let str1 = "hello"
let str2 = " wasim"

let str3 = str1 + str2
// console.log(str3);

// console.log("1" + 2);
// console.log(1 + "2");
// console.log("1" + 2 + 2);
// console.log(1 + 2 + "2"); Types of conversion

// console.log(3 + 4 * 5 % 3); // donot massy code like this 


// console.log(+true); // output 1
// console.log(+"");  // output 0

let num1, num2, num3
num1 = num2 = num3 = 2 + 2 // don't code like this 

let gameCounter = 100
++gameCounter;   // prefix increament
// gameCounter++; // postfix increament
console.log(gameCounter);

// link to study prefix and postfix
// https://tc39.es/ecma262/multipage/abstract-operations.html#sec-type-conversion



